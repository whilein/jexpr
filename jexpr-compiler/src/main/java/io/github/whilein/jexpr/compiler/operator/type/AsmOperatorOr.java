...
public final class AsmOperatorOr extends AbstractAsmBinaryOperator {
    ...
    public AsmOperatorOr(Class<? extends BinaryOperator> operatorType) {
        super(operatorType);
    }
    ...
    public void compile(
        ...
        compiler.visitJumpInsn(Opcodes.IFNE, trueLabel);

        right.load();
        compiler.unbox(right.getType());

        compiler.visitJumpInsn(Opcodes.IFEQ, falseLabel);

        compiler.visitLabel(trueLabel);
